...
import {
  ...
  GET_CONTACTS_FAIL,
  GET_HOME_ADMITTANCEDATE_SUCCESS,
  GET_HOME_ADMITTANCEDATE_FAIL,
  GET_HOME_STUDY_START_SUCCESS,
  GET_HOME_STUDY_START_FAIL,
} from './types'
...
export const load_home_dates = () => async dispatch => {
  const config = {
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
    ...
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/admittancedate/`, config);

      dispatch({
        type: GET_HOME_ADMITTANCEDATE_SUCCESS,
        payload: res.data[0],
      })
    } catch (err) {
      dispatch({
        type: GET_HOME_ADMITTANCEDATE_FAIL
      })
    }
}
